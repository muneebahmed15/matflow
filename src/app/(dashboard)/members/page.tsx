...
export default function MembersPage() {
  ...
  const beltColor: Record<string, string> = {
    ...
    brown: 'bg-amber-700/20 text-amber-600',
    ...
    <div className="p-6 md:p-8 max-w-5xl mx-auto">
      ...
      {filtered.length === 0 ? (
        <div className="bg-white/5 border border-white/10 rounded-2xl p-16 text-center">
          ...
          <Link href="/members/new" className="mt-4 inline-block text-blue-400 text-sm hover:underline">
            ...
            <tbody className="divide-y divide-white/5">
              {filtered.map((m) => (
                <tr key={m.id} className="hover:bg-white/3 transition">
                  ...
                  <td className="px-5 py-3">
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs font-medium ${m.status === 'active' ? 'bg-green-500/10 text-green-400' : 'bg-white/5 text-white/30'}`}
                    >
                      {m.status}
                    </span>
                  </td>
                  <td className="px-5 py-3 text-right">
                    <Link href={`/members/${m.id}`} className="text-blue-400 hover:underline text-xs">
                      View →
                    </Link>
                  </td>
                </tr>
