...
async function updateSession(request: NextRequest) {
  ...
  if (user) {
    ...
    if (isOnboarding || isSetup) {
      if (personas.hasStaff && isSetup) {
        return supabaseResponse;
      }
      if (personas.hasStaff) return redirectTo(request, '/dashboard');
      if (personas.hasMember && !personas.hasStaff) return redirectTo(request, '/portal');
      return supabaseResponse;
    }
